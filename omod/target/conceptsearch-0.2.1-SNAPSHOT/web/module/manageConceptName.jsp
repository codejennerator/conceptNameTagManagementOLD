	...
	<tr>
		...
		<div id="divClass" style="display:none;">
			<c:forEach var="class" items="${conceptClasses}">
				<input type="checkbox" <c:if test="${fn:contains(conceptSearch.conceptClasses, class)}"> checked </c:if> name="conceptClasses" value="${class.id}">org.apache.maven.project.MavenProject<br />
			</c:forEach>
		</div>
		</td>
	</tr>
...
<div style="width:74%;  overflow: hidden; float:right">
  <openmrs:portlet url="searchresultAdvManagement" id="searchresultAdvManagement" moduleId="conceptsearch" />
</div>
